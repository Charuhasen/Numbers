...
| `daily_challenges` | Daily challenge seed and metadata. | Read all. Admin insert only. |
...
### 2.7 `daily_challenges`

Stores the daily challenge configuration so all players get the same experience.

```sql
create table public.daily_challenges (
  id uuid default gen_random_uuid() primary key,
  challenge_date date unique not null,   -- The date this daily is for (UTC)
  seed int not null,                     -- Random seed for deterministic grid generation
  challenge_sequence jsonb not null,     -- Array of 10 challenge IDs or inline challenge configs
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- Admin insert only (generated by a scheduled job or admin tool).
```
...
begin
  ...
  -- For daily mode, only show today's scores
  if p_mode = 'daily' then
    v_daily_filter := current_date;
  end if;
  ...
  from (
    ...
    from (
      ...
      where s.mode = p_mode
        and (v_daily_filter is null or s.played_at::date = v_daily_filter)
